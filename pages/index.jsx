...
import { getFirstError, sum } from "utils/utils";
...
const Index = () => {
    ...
    const fetchReport = async (from, to) => {
        setLoading(true);
        try {
            const { data: { data } } = await axiosInstance.get(`${config.apiGateway.API_URL_DUMMY}/dashboard/b2b/overview`, {
                params: {
                    from: moment(from).format("YYYY-MM-DD"),
                    to: moment(to || moment().toDate()).format("YYYY-MM-DD"),
                },
            });
            const totals = [
                sum(data, "totalCustomers"),
                sum(data, "totalCustomersSave"),
                sum(data, "totalBookings"),
                sum(data, "totalRemainingBalance"),
            ];
            const min = -4;
            const max = 5;
            setTableData(prev => {
                const updatedData = prev.map((p, i) => {
                    return {
                        ...p,
                        percentage: ((Math.random() * (max - min)) + min).toFixed(2),
                        count: totals[i]
                    }
                })
                return updatedData
                    ...
                    tableData.map((card, idx) => <React.Fragment key={`${card.count}-${idx}`}>
                        <div className='p-4 bg-white rounded-lg shadow-sm dark:bg-slate-800'>
                            ...
                            <div className="flex mt-3">
                                ...
                                <span dir="ltr" className="mx-2 font-thin text-gray-300">{moment(startDate || moment().startOf("month").toDate()).fromNow()}</span>
                            </div>
                        </div>
                    </React.Fragment>)
                }
