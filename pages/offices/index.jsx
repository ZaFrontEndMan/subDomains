...
import { officesColumns } from "components/columns";
...
import {
  ...
  Filter,
  UpdatePricesModal,
} from "components/pages/admin/offices-list";
...
const Index = ({ companyUUID }) => {
  ...
  const activation = async (uuid, activate) => {
    ...
    try {
      await axiosInstance.put(
        ...
        }/v2/supplier/business/office/${uuid}/${
  ...
  const columns = officesColumns(t, activation, actionLoading);
  const fetchReport = async (page, perPage, query = "", filter = {}) => {
    ...
    try {
      ...
      const { data } = await axiosInstance.post(
        `${config.apiGateway.API_URL_TELGANI}/v2/supplier/business/company/${companyUUID}/office/list`,
        {
        ...
        },
        {
          params: {
            "search[value]": search,
            ...(_filter?.isActive?.value != null
              ? { "columns[2][search][value]": _filter?.isActive?.value }
              : {}),
          },
        }
      );
  ...
  const handleExportExcel = async () => {
    ...
    await exportExcel(tableData, columns, `${t("offices_key")}`, handleMessage);
    setTimeout(() => {
    ...
    <>
      <div className="min-h-full bg-gray-100 rounded-md dark:bg-gray-700">
        <Header
          ...
          path="/companies-list"
          links={[{ label: t("offices_key"), path: "/offices-list" }]}
...
export const getServerSideProps = async (context) => {
  ...
  const companyUUID = context.query?.com || "";
