...
export default function CompanySearch({ companyId, fetchReport, ...reset }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await axiosInstance.get(`${config.apiGateway.API_URL_TELGANI}/v2/supplier/business/company/collection`)
        .then(({ data }) => {
...
//       axiosInstance
//         .get(
//           `${config.apiGateway.API_URL_TELGANI}/v2/supplier/business/company/collection?search[value]=${inputValue}&start=0&length=10&is_export=false`
//         )
//         .then(({ data }) => {
